...
func _ready():
	open()
	show_chart(LevelsManager.save_bird_count, LevelsManager.not_save_bird_count)
...
func show_chart(save: int, not_save: int):
	# Let's create our @x values
	var x: Array = [save, not_save]
	
	# And our y values. It can be an n-size array of arrays.
	# NOTE: `x.size() == y.size()` or `x.size() == y[n].size()`
	var y: Array = ["Спасти птицу", "Оставить птицу"]
	...
	cp.title = "Выборы игроков в эпизоде"
